...
export const EN_DICT: Record<string, string> = {
  ...
  "呼入中继": "Inbound trunks",
  "添加中继": "Add trunk",
  "呼入中继可以通过 SIM 卡拨出，产生真实话费。只填写确实需要的目标号码和 SIM 卡。": "An inbound trunk can dial out through a SIM and spend real money. List only the destinations and SIMs it actually needs.",
  "这些设备已不存在，使用它们的中继在拨号时会失败": "These devices no longer exist; trunks using them will fail at dial time",
  "没有呼入中继。外部 SIP 对端无法接入。": "No inbound trunks. No external SIP peer can reach this system.",
  "对端地址": "Peer address",
  "允许的来源地址或网段": "Allowed source addresses or CIDRs",
  "用户名（可选）": "Username (optional)",
  "密码（保存后不再回显）": "Password (never shown again after saving)",
  "允许拨打的号码模式，如 _1NXXNXXXXXX": "Permitted destination patterns, e.g. _1NXXNXXXXXX",
  "允许使用的 SIM，用空格分隔": "Permitted SIMs, space separated",
  "同时允许使用共享外呼路由（这些呼叫不受上面的 SIM 限制和并发上限约束）": "Also allow the shared outbound routes (those calls escape the SIM list and the cap above)",
  "必须填写来源地址或凭据之一；目标模式以 _ 开头；并发上限与超时单位为秒": "A source address or credentials is required; destination patterns start with _; the cap is calls and the timeout is seconds",
