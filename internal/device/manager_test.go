...
// The real reply from a Quectel module on a China Mobile SIM. The BCD filler
// nibble lands mid-string here, not at the end, so trimming only the right
// side left it in place, the digit check failed, and the ICCID read as absent
// -- which disables the whole card policy panel, the VoWiFi switch included.
func TestParseICCIDStripsAnInteriorBCDFiller(t *testing.T) {
	cases := []struct {
		name     string
		lines    []string
		expected string
		...
		{
			"interior filler",
			[]string{"+CCID: 898600910121F0105571", "OK"},
			"8986009101210105571",
		},
		{
			// The case the original TrimRight was written for.
			"trailing filler",
			[]string{"+CCID: 8986009101210105571F", "OK"},
			"8986009101210105571",
		},
		{
			"lowercase filler",
			[]string{"+QCCID: 898600910121f0105571", "OK"},
			"8986009101210105571",
		},
		{
			// No filler at all: a full 20-digit ICCID must survive untouched.
			"twenty digits, no filler",
			[]string{"+CCID: 89860091012101055717", "OK"},
			"89860091012101055717",
		},
		{
			"quoted",
			[]string{`+CCID: "898600910121F0105571"`, "OK"},
		...
		{
			// Bare digits with no prefix reach the line scan.
			"no prefix",
			[]string{"898600910121F0105571", "OK"},
			"8986009101210105571",
		},
	}
	for _, test := range cases {
		got := parseICCIDIdentifier(
			modem.Response{Lines: test.lines}, []string{"+CCID:", "+QCCID:"}, 18, 22)
		if got != test.expected {
			t.Fatalf("%s: parseICCIDIdentifier = %q, want %q", test.name, got, test.expected)
		}
	}
}

// Stripping F must not invent an ICCID out of a reply that has none, or the
// warning this pairs with would never fire and the panel would show a bad
// value instead of an honest absence.
func TestParseICCIDStillRejectsNonsense(t *testing.T) {
	for _, lines := range [][]string{
		{"+CCID:", "OK"},
		{"OK"},
		{"+CCID: FFFFFFFFFFFFFFFFFFFF", "OK"},
		{"+CCID: 8986", "OK"},                 // too short
		{"+CCID: 898600910121A0105571", "OK"}, // a non-F non-digit
		{"ERROR"},
	} {
		if got := parseICCIDIdentifier(
			modem.Response{Lines: lines}, []string{"+CCID:", "+QCCID:"}, 18, 22); got != "" {
			t.Fatalf("parseICCIDIdentifier(%v) = %q, want empty", lines, got)
		}
	}
}
